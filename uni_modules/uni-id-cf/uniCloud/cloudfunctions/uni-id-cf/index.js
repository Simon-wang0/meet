...
exports.main = async (event, context) => {
    ...
    */
    ...
    //判断该用户是否是第一次登录 ，第一次需要进入 新手指导页面
    const isNeedGuide = async () => {
        let isNew = await uniIdLogCollection.where({
            //当前用户id
            _id: params.uid,
            type: 'login'
        }).get()
        return isNew.data.length === 0
    }
    ...
    switch (action) { //根据action的值执行对应的操作
        ...
        case 'login':
            ...
            let needGuide = await isNeedGuide();
