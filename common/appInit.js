    ...
    uniCloud.addInterceptor('callFunction', {
        ...
        success: (e) => {
            ...
            switch (e.result.code) {
                case 0:
                    //将心仪信息保存到缓存里
                    let id = e.result.userInfo.favorite_person
                    console.log("id==",id)
                    if (id != '' && id != null) {
                        db.collection('uni-id-users')
                            ...
                            .get().then((res) => {
                                console.log("res", res)
                                store.commit('getFavoriteId/changeIsEdit')
                                const favoriteInfo = res.result.data[0]
                                uni.setStorageSync('favoriteInfo', favoriteInfo);
                                console.log('心仪人信息放入缓存成功', favoriteInfo);
                    ...
                    const dur = new Date().getTime() - e.result.userInfo.register_date
                    if (dur < 1000 * 60) {
                        uni.navigateTo({
                            url: "/pages/new-person-guide/personal-info/personal-info"
                        })
                    }
                    break;
                case 403:
